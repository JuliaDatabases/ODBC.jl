...

{
    ...
    "title": "ODBC.DSN",
    "category": "section",
    "text": "Constructors:ODBC.DSN(dsn, username, password) => ODBC.DSN ODBC.DSN(connection_string) => ODBC.DSN ODBC.disconnect!(dsn::ODBC.DSN)The first method attempts to connect to a pre-defined DSN that has been pre-configured through your system's ODBC admin console. Settings such as the ODBC driver, server address, port #, etc. are already configured, so all that is required is the username and password to connect.The second method takes a full connection string. Connection strings are vendor-specific, but follow the format of key1=value1;key2=value2... for various key-value pairs, typically including Driver=X and Server=Y. For help in figuring out how to build the right connection string for your system, see connectionstrings.com.ODBC.disconnect!(dsn) can also be used to disconnect."
},
