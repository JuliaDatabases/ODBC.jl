...
function ODBCConnect!(dbc::Ptr{Void},dsn::ASCIIString,username::ASCIIString,password::ASCIIString)
...
function ODBCDriverConnect!(dbc::Ptr{Void},conn_string::ASCIIString,driver_prompt::Uint16)
...
# Send query to DMBS
function ODBCQueryExecute(stmt::Ptr{Void}, querystring::ASCIIString)
...
function ODBCMetadata(stmt::Ptr{Void},querystring::ASCIIString)
        ...
        colnames = ASCIIString[]
        coltypes = Array((ASCIIString,Int16),0)
        colsizes = Int[]
        coldigits = Int16[]
        colnulls  = Int16[]
...
ODBCAllocate(x::Array{Uint8,2},y)        = Array(ASCIIString,y)
...
ODBCClean(x,y,z) = x[y]
ODBCClean(x::Array{Uint8},y,z) = ascii(x[1:z,y])
...
function ODBCCopy!(dest::Array{ASCIIString},dsto,src::Array{Uint8,2},n,ind,nas)
    for i = 1:n
        nas[i+dsto-1] = ind[i] < 0
        dest[i+dsto-1] = ascii(bytestring(src[1:ind[i],i]))
    end
end

function ODBCCopy!(dest::Array{ASCIIString},dsto,src::Array{Uint32},n,ind,nas)
    for i = 1:n
        nas[i+dsto-1] = ind[i] < 0
        dest[i+dsto-1] = ascii(bytestring(convert(Array{Uint8},src[1:div(ind[i],4),i])))
    end
end

# ODBCEscape takes a Julia value and gets it ready for writing to a file i.e. converts to string
ODBCEscape(x) = string(x)
ODBCEscape(x::ASCIIString) = "\"$x\""
...
function ODBCDirectToFile(stmt::Ptr{Void},meta::Metadata,columns::Array{Any,1},rowset::Int,output::ASCIIString,delim::Char,l::Int)
